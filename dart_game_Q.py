...
# Dart properties (common for both levels)
dart_x = 150
dart_y = HEIGHT // 2
dart_size = 6
...
dart_speed = 8.8  # Increased by 10% from 8
...
def draw_hit_effect(hit_x, hit_y, points):
    """Draw a visual effect when the dart hits the dartboard"""
    ...
    for r in range(max_radius, 0, -2):
        alpha = int(150 * (r / max_radius))
        pygame.draw.circle(effect_surface, (color[0], color[1], color[2], alpha), (int(hit_x), int(hit_y)), r)
    
    return effect_surface

def draw_score_popup(hit_x, hit_y, points):
    """Draw a score popup at the hit location"""
    if points > 0:
        font = pygame.font.SysFont(None, 36)
        text = font.render(f"+{points}", True, (255, 255, 0))
        text_rect = text.get_rect(center=(hit_x, hit_y - 30))
...
def create_obstacles():
    ...
    y_pos1 = HEIGHT // 2 - 17  # Positioned 7px higher (was -10)
...
def draw_timer():
    ...
    if current_level == 3 and not game_over:
        ...
        draw_rounded_rect(screen, (0, 0, 0, 150), bg_rect)
...
def draw_dart(x, y, angle):
    """Draw the dart at the specified position and angle"""
...
def draw_previous_trajectories():
    """Draw trajectories of previous throws"""
    ...
    for i, (angle, hit_x, hit_y, hit_center_y) in enumerate(previous_trajectories):
        ...
        pygame.draw.line(line_surface, color, (start_x, start_y), (end_x, end_y), 1)
        screen.blit(line_surface, (0, 0))
        
        # Draw a small circle at the hit point
        pygame.draw.circle(line_surface, (255, 255, 255, 150), (int(end_x), int(end_y)), 3)
        screen.blit(line_surface, (0, 0))
...
def draw_score_box():
    ...
    draw_rounded_rect(screen, (40, 40, 40, 200), box_rect)
    
    font = pygame.font.SysFont(None, 28)
    
    # Level indicator
    level_text = font.render(f"Level {current_level}", True, (255, 220, 100))
    ...
    # Level goal
    if current_level == 1:
        goal_text = font.render("Goal: 70 pts", True, (150, 255, 150))
        screen.blit(goal_text, (box_rect.x + 15, box_rect.y + 75))
    elif current_level == 2:
        goal_text = font.render("Goal: 60 pts", True, (150, 255, 150))
        screen.blit(goal_text, (box_rect.x + 15, box_rect.y + 75))
...
def draw_start_screen():
    ...
    if current_level == 1:
        instructions = [
            ...
            "4. Hit the middle ring (gold) for 30 points",
            "5. Hit the outer ring (blue) for 10 points",
...
def main():
    ...
    while running:
        ...
        # Update obstacles for level 3 (no wind updates)
        if current_level == 3:
            update_obstacles()
        ...
        if score_popup and score_popup_timer > 0:
            ...
            rect.y -= 0.5
            alpha = int(255 * (score_popup_timer / 60))
